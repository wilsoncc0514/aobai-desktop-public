...
当前快照：`mac-test-20260904`。程序版本元数据仍为 `0.4.1`，不是新的正式版本号。

**Windows 本轮未实机测试。** 已有源实现通过 macOS/Windows 自动测试与构建，但 CI 不代表真实桌面的透明窗口、托盘、拖动、缩放等行为已验收。

## 功能

- 透明无边框的208×208窗口，支持置顶、普通和置底层级。
- 常态正面蹲坐、轻摆尾；按下翻肚皮撒娇，按住可拖动，松手收势回坐，悬停不触发。
...
- 用户已在主要Mac上确认按住拖动和动作衔接；其他Mac、混合DPI和Windows实机仍需测试。
